...
*- Cost и Root ID сменились на 18 на свиче S3, следовательно через него можно "быстрее" добраться до коммуттаора корнегвого моста. Т.к. первым при выборе сравнивается Cost и только потом BID. На коммутаторе S1 приорет порта Fa0/4 выше, поэтому он и блокируется.*  
