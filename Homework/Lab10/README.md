...
*- Маршрутизатор с самым высоким приоритетом OSPF и самым высоким Router ID выбирается как DR.  
...
*- Стоимость маршрута по умолчанию (0.0.0.0/0) в OSPF отличается, потому что это внешний маршрут, Е2. 
При использовании E2 маршрута, значения метрик не складываются, а используется только значение метрики объявленное из внешней сети (метрика редистрибуции), в нашем случае получается метрика = 1
В то время как внутренние маршруты расчитываются с помощью алгоритма Дейкстры.*
